...
export function MiNominaPage() {
      ...
      try {
        ...
        try {
          ...
          if (empleadoResponse.data.success && empleadoResponse.data.empleado) {
            console.log("Empleado encontrado:", empleadoResponse.data.empleado);
            setUserData({
              ...user,
              empleado_id: empleadoResponse.data.empleado._id,
  ...
  useEffect(() => {
    const fetchNominas = async () => {
      ...
      const urls = [
        `http://localhost:4000/api/nominas/empleado/${userData.empleado_id}`,
        `http://localhost:4000/nominas/empleado/${userData.empleado_id}`,
        `http://localhost:4000/api/nominas-api/empleado/${userData.empleado_id}`,
        `http://localhost:4000/nominas-api/empleado/${userData.empleado_id}`
      ];
      ...
      console.error("No se pudieron obtener nóminas:", error);
  ...
  const verPDF = (nomina) => {
    ...
    if (nomina.url) {
      ...
      const altUrl = `http://localhost:4000/nomina/download/${nomina._id}?name=${empleadoName}&fecha=${fecha}`;
