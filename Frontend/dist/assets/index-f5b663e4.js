  ...
  const verPDF = async (nomina) => {
    try {
      ...
      const response = await axios({
        url: `${"https://lead-inmobiliaria.com:4000"}/api/nominas-api/download/${nomina._id}`,
        method: "GET",
        responseType: "blob",
        withCredentials: true
      });
...
function MisProyectos() {
  ...
  const descargarPDF = async (captacion) => {
    try {
      ...
      const response = await fetch(`${"https://lead-inmobiliaria.com:4000"}/api/captaciones/${captacion._id}/pdf`, {
        method: "GET",
        credentials: "include"
      });
      if (!response.ok) {
